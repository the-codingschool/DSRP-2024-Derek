...
#####Significant difference in sentiment scores between characters across seasons:
...
#####Significant difference in relationship between avg imdb rating and avg sentiment:
...
#####Do episodes with a certain writers have different IMDb ratings?
...
#####Significant difference in sentiment scores for Jim across seasons
...
#####Do episodes with certain writers have different IMDb ratings?
...
#####Predict IMDb rating based on the involvement of Greg Daniels as a writer.
...
#####Impact of each writer on IMDB score:
...
imdbgraph <- plot_ly(
  ...
  layout(
    ...
    paper_bgcolor = "#f2e9dd", # Overall plot background
    plot_bgcolor = "#FFFFFE", # Plot area background
    showlegend = TRUE
  )
